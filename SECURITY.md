...
**Status**: pure-Go library + offline-by-default CLI (Phase-1 MVP, inception 2026-05-27; stele-anchor amendment 2026-06-11) — FDA 21 CFR Part 11 clinical-trial audit-trail forge in `cmd/trial-ledger` (one CLI binary) + 9 internal packages (`internal/{auditledger,fdacfr11,firewall,honest,legal,lore,manifest,mirrormark,stele}`). **No HTTP listener, no daemon, no auth surface, no DB.** ONE opt-in outbound HTTP client exists since the 2026-06-11 stele-anchor amendment: `internal/stele` (Stele verified-trust-spine anchoring; client-only, 5s timeout, dead code unless `TRIAL_LEDGER_STELE_URL` is set — pinned by `TestR145B_SteleAnchorConfinement`). The CLI reads three env vars — two at boot for the Mirror-Mark marker (`TRIAL_LEDGER_LORE_CORPUS_SHA_PATH` + `TRIAL_LEDGER_MIRRORMARK_KEY`) plus the opt-in `TRIAL_LEDGER_STELE_URL` — and reads / writes newline-delimited JSON on stdin / stdout. This file documents the trust boundaries that DO exist so future agents can verify what is and is not part of the threat model.
...
- **Primary language**: Go 1.22; `cmd/trial-ledger` is an offline-by-default CLI (`append` reads stdin / writes stdout; `advisories` / `version` / `manifest` / `legal` are pure prints). No HTTP listener, no auth, no daemon mode. The only network code path is the opt-in outbound Stele anchor (`append` + `TRIAL_LEDGER_STELE_URL` set).
- **Planned sibling (R169)**: Rust 1.78+ via `trial-ledger-rust` — byte-identical wire shape; this file applies to both substrates.
- **Go dependencies**: zero (verified by `cat go.mod` — only `module github.com/davly/trial-ledger` + `go 1.22`; no `require` block, no `go.sum`). `internal/stele` is a deliberately LOCAL stdlib-only mirror of the spine wire-contract — the stele Go module is NOT imported.
- **Shape**: pure library — `internal/` packages are deterministic pure functions of their inputs; `cmd/trial-ledger` is a one-shot CLI reading `os.Args` + stdin + three env vars only (the third, `TRIAL_LEDGER_STELE_URL`, is opt-in and confined to one `os.Getenv(stele.EnvURL)` site by the R145.B firewall pin).
...
## Stele spine anchoring (opt-in, 2026-06-11 amendment)

The `append` command can anchor its run's audit ledger into the Stele verified-trust spine (`infrastructure/stele`) via `POST /v1/verdicts`. The boundary shape:

- **Off by default**: unset/empty `TRIAL_LEDGER_STELE_URL` = disabled = no self-check, no HTTP, no new output — behavior identical to a non-anchoring build. Outbound-only; nothing ever listens.
- **Self-check gate (honesty contract)**: the anchor verdict is LIT only after `auditledger.SelfCheck()` passes (re-derive every row's Mirror-Mark via the ledger's own marker + re-run the closed-enum R115 / required-field structural checks). A failed self-check seals NOTHING and the CLI exits non-zero. `oracle_strength` is `"Self-Check"` and the evidence sentence says explicitly the oracle is trial-ledger's own verifier, NOT an independent gauntlet.
- **subject_hash binding**: sha256 hex over the canonical run serialization (JSONL of `Record` in append order — documented at `auditledger.(*Ledger).SelfCheck`).
- **Never-overclaim**: the `stele anchor: sealed seq=N entry_hash=…` receipt (stderr, keeping stdout a pure NDJSON Record stream) prints only after the spine returned 201 WITH an entry_hash; a requested-but-failed anchor (self-check / network / non-201) prints to stderr and exits non-zero.
- **Complements, does not replace, Mirror-Mark**: the FDA reviewer's offline cold-verify against (corpusSHA, key) per §11.10(e) needs neither trial-ledger nor the spine; the anchor adds an independent tamper-evident timestamped seal over the whole-run ledger digest.
- **Firewall pins**: `TestR145B_SteleAnchorConfinement` (net/http confined to `internal/stele` client-only; listener primitives banned everywhere; env reads confined to the two mirrormark boot reads + one `os.Getenv(stele.EnvURL)` in `cmd/trial-ledger/main.go`; wire-contract constants pinned) + `stele` registered in the R145.C `ExpectedPackages` list (8 → 9).
